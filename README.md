...
* yt-dlp uses modern http client backends such as `curl-cffi` (preferred) and `requests`. Use `--compat-options prefer-legacy-http-handler` to prefer the legacy http handler (`urllib`) to be used for standard http requests.
